...
from app.simulation.world_state import WorldState, clamp
...
def update_economy(state: WorldState) -> None:
    ...
    for company in state.companies:
        ...
        labor_signal = district_productivity[company.district_id]
        ...
        tax_drag = state.government.tax_rate * 0.07
        ...
        company.capital = round(max(0, company.capital * (1 + growth) + public_contracts), 2)
        ...
        company.wage = round(max(18_000, company.wage * (1 + (labor_signal - 0.54) * 0.006 + state.metrics["inflation"] * 0.018)), 2)
        ...
        gdp += company.employees * company.wage * company.productivity / 52
